...
class Agglomerative :
    def __init__(self,image_path) -> None:
        ...
        self.image = np.array(cv.cvtColor(self.original_image,cv.COLOR_RGB2LUV))
        self.points = self.image.reshape(self.image.shape[0] * self.image.shape[1] , 3)
        self.distance_matrix = [[0]*len(self.points) for i in range(len(self.points))]
    ...
    def calcualte_distance(self,p1,p2) :
        return float("{:.2f}".format(np.sqrt(np.sum(np.square(np.subtract(p1,p2)))),2))
...
agg = Agglomerative("../images/beach.jpg")
agg.image_mask()
# agg.fit(2)
# agg.imageShow()
